...
ns = Namespace('topics', description='专题')
topic_more_model = ns.model('TopicsMoreModel', {
    'start': fields.Integer(enum=[0, 20], description='The topic unique identifier'),
    'limit': fields.Integer(required=True, default=16, min=16, description='最少16，且是8的倍数'),
})
...
class TopicsMore(Resource):
    ...
    @ns.expect(topic_more_model)
    def post(self):
        """获取更多的专题"""
        postData = request.get_json()
        start = postData.get('start')
        limit = postData.get('limit')
        if not (start % 8 == 0 and limit > 16):
